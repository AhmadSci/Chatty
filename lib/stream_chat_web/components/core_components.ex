defmodule StreamChatWeb.CoreComponents do
  ...
  def input(%{type: "checkbox"} = assigns) do
    ...
    ~H"""
    <label phx-feedback-for={@name} class="flex items-center gap-4 text-sm leading-6 text-zinc-600" style = "color:white">
  ...
  def label(assigns) do
    ~H"""
    <label style = "color:white" for={@for} class="block text-sm font-semibold leading-6 text-zinc-800">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-lg font-semibold leading-8 text-zinc-800" style ="color:white">
          <%= render_slot(@inner_block) %>
        </h1>
        <p :if={@subtitle != []} class="mt-2 text-sm leading-6 text-zinc-600" style ="color:#747474">
          <%= render_slot(@subtitle) %>
        </p>
      </div>
